...
The installers read chart versions from `/srv/config/chart-versions.yaml` by default.
...
- chart versions are sourced from `/srv/config/chart-versions.yaml`
- active overrides live under `/etc/platform-config/helm-configs/`
- PoundCake-specific overrides live under `/srv/config/poundcake/`
- global overrides live under `/etc/platform-config/helm-configs/global_overrides/`
...
- Active override directory: `/srv/config/poundcake/`
- Split files used in the documented rebuild:
  - `/srv/config/poundcake/00-pull-secret-overrides.yaml`
  - `/srv/config/poundcake/10-main-overrides.yaml`
  - `/srv/config/poundcake/20-auth-overrides.yaml`
  - `/srv/config/poundcake/30-git-sync-overrides.yaml`
- Global overrides: `/etc/platform-config/helm-configs/global_overrides/*.yaml`
- Chart version file: `/srv/config/chart-versions.yaml`
...
The installer reads chart versions from `/srv/config/chart-versions.yaml`.
...
Recommended live file layout under `/srv/config/poundcake/`:
...
These are the files that actually drive the install. The documented operator flow edits `/srv/config/poundcake/` directly.
...
PoundCake accepts either `Authorization: Bearer <internal-api-key>` or `X-Auth-Token: <internal-api-key>` for internal service calls. The Prometheus Operator path above matches the working integration used on `deployment-host`.
...
1. Create a temporary override file such as `/srv/config/poundcake/90-live-core-validation-overrides.yaml`:
...
1. Create or update the active Genestack override files under `/srv/config/poundcake/`.
2. Put the Bakery runtime settings in `/srv/config/poundcake/10-main-overrides.yaml`.
3. Put pull-secret references, if needed, in `/srv/config/poundcake/00-pull-secret-overrides.yaml`.
4. Install Bakery:
...
1. Create or update the active Genestack override files under `/srv/config/poundcake/`.
2. Put the PoundCake runtime settings in `/srv/config/poundcake/10-main-overrides.yaml`.
3. Put pull-secret references, if needed, in `/srv/config/poundcake/00-pull-secret-overrides.yaml`.
4. Install PoundCake:
...
2. Update `/srv/config/poundcake/10-main-overrides.yaml` for the PoundCake release.
...
4. Update the active PoundCake override file at `/srv/config/poundcake/10-main-overrides.yaml`.
5. Set:
...
Put this in `/srv/config/poundcake/00-pull-secret-overrides.yaml`.
