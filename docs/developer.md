...
Note:
- `install/install-helm.sh` reads desired chart version from `/srv/config/chart-versions.yaml` key `poundcake`.
...
| `POUNDCAKE_VERSION_FILE` | `/srv/config/chart-versions.yaml` | Optional | Source for auto-detected chart version key `poundcake` | Change only if your version file is elsewhere |
...
helm pull oci://ghcr.io/${FORK_OWNER}/charts/poundcake-standalone --version 0.0.2
```
...
- Chart: `oci://ghcr.io/<owner>/charts/poundcake-standalone`
