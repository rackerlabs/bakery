...
- The documented examples assume a Genestack host layout under `/etc/platform-config/` and a pre-existing Gateway named `flex-gateway` in namespace `envoy-gateway`.
- The live values used by the installers come from `/srv/config/poundcake/`, not from repo example files.
