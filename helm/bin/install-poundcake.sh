...
BASE_OVERRIDES="${POUNDCAKE_BASE_OVERRIDES:-/opt/platform-config/base-helm-configs/poundcake/poundcake-helm-overrides.yaml}"
GLOBAL_OVERRIDES_DIR="${POUNDCAKE_GLOBAL_OVERRIDES_DIR:-/etc/platform-config/helm-configs/global_overrides}"
SERVICE_CONFIG_DIR="${POUNDCAKE_SERVICE_CONFIG_DIR:-/srv/config/poundcake}"
POST_RENDERER="${POUNDCAKE_HELM_POST_RENDERER:-/etc/platform-config/kustomize/kustomize.sh}"
POST_RENDERER_ARGS="${POUNDCAKE_HELM_POST_RENDERER_ARGS:-poundcake/overlay}"
POST_RENDERER_OVERLAY_DIR="${POUNDCAKE_HELM_POST_RENDERER_OVERLAY_DIR:-/etc/platform-config/kustomize/poundcake/overlay}"
...
apply_bakery_rackspace_secret() {
  ...
  if [[ "${INTERACTIVE_BAKERY_CREDS}" == "true" ]]; then
    ...
    local prompt_url_default="${BAKERY_RACKSPACE_URL:-https://core.example.com}"
...
apply_bakery_db_integrated_overrides() {
  if [[ "${BAKERY_DB_INTEGRATED}" != "true" ]]; then
    return 0
  fi
  ...
  if [[ -z "${BAKERY_DB_HOST}" || -z "${BAKERY_DB_NAME}" || -z "${BAKERY_DB_USER}" ]]; then
    log_error "Integrated Bakery DB mode requires non-empty host, name, and user."
    exit 1
  fi
...
resolve_chart_version() {
  ...
  candidate_files+=(
    "/etc/platform-config/helm-chart-version.yaml"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --bakery-db-user)
      ...
      shift 2
      ;;
    --bakery-db-password|--bakery-db-password-secret-name|--bakery-db-password-secret-key|--bakery-db-admin-secret-name|--bakery-db-admin-password-key|--bakery-db-sql-image)
      log_error "Option '$1' was removed. Configure integrated Bakery DB bootstrap via chart values (bakery.database.*)."
      exit 1
      ;;
    *)
      EXTRA_ARGS+=("$1")
...
if [[ "${NO_LOCAL_BAKERY}" != "true" ]]; then
  ...
  log_info "Skipping integrated Bakery DB Helm overrides because no-local-bakery mode is enabled."
fi
