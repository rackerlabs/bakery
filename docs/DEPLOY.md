...
Image repositories/tags/digests must be configured in Helm values or override files such as `/srv/config/poundcake/poundcake-helm-overrides.yaml`, not installer env vars.
...
- Chart versions are sourced from `/etc/platform-config/helm-chart-version.yaml` and `/srv/config/chart-versions.yaml`.
...
bakery:
  gateway:
    ...
    listener:
      name: bakery-https
      hostname: bakery.api.ord.example-domain.net
      port: 443
      protocol: HTTPS
      ...
      updateIfExists: true
    hostnames:
      - bakery.api.ord.example-domain.net
```
...
bakery:
  ...
  client:
    enabled: true
    baseUrl: "https://bakery.api.ord.example-domain.net"
    auth:
      mode: hmac
      existingSecret: bakery-hmac
      secretKeys:
...
- `/etc/platform-config/helm-configs/stackstorm/*.yaml`
...
- To customize: add override files under `/etc/platform-config/helm-configs/stackstorm/*.yaml` to re-enable optional services or increase replicas.
