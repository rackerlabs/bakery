...
The installers resolve chart version from `/srv/config/chart-versions.yaml` by default. Configure image repositories/tags/digests in Helm values or override files such as `/srv/config/poundcake/poundcake-helm-overrides.yaml`, not installer env vars. For the opinionated step-by-step deployment process, see [docs/REMOTE_BAKERY_DEPLOYMENT_GUIDE.md](docs/REMOTE_BAKERY_DEPLOYMENT_GUIDE.md).
