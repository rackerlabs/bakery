...
Image repositories/tags/digests:
  ...
  - Default override path: /srv/config/poundcake/poundcake-helm-overrides.yaml
...
validate_image_env_inputs() {
  ...
  if (( ${#deprecated_image_envs[@]} > 0 )); then
    ...
    log_error "Default override path: /srv/config/poundcake/poundcake-helm-overrides.yaml"
    exit 1
  fi
}
