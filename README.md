...
Installer chart versions are read from `/srv/config/chart-versions.yaml`:
- `poundcake`
- `stackstorm`
- `mariadb-operator`
- `redis-operator`
...
- Override behavior: add values files under `/etc/platform-config/helm-configs/stackstorm/*.yaml` to re-enable components or scale replicas above 1.
