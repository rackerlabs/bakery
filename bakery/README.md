...
- If `coreQueue` is not provided, Bakery uses `RACKSPACE_CORE_DEFAULT_QUEUE` (default `Support Queue`).
...
| `RACKSPACE_CORE_DEFAULT_QUEUE` | Rackspace Core | Default queue used when `coreQueue` is not provided (default: `Support Queue`) |
