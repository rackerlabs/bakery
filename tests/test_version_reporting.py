...
def test_resolve_version_uses_first_configured_env_var(monkeypatch) -> None:
    monkeypatch.setenv("PRIMARY_VERSION", "0.1.9")
    monkeypatch.setenv("SECONDARY_VERSION", "0.1.8")

    assert resolve_version("PRIMARY_VERSION", "SECONDARY_VERSION") == "0.1.9"
...
def test_bakery_settings_app_version_uses_runtime_env(monkeypatch) -> None:
    monkeypatch.setenv("BAKERY_APP_VERSION", "0.1.9")

    settings = BakerySettings()

    assert settings.app_version == "0.1.9"
