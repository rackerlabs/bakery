...
if [ -f "/srv/config/chart-versions.yaml" ]; then
    VERSION=$(grep "^stackstorm:" /srv/config/chart-versions.yaml | awk '{print $2}')
fi
VERSION="${VERSION:-}"
...
# Add post-renderer if available (for platform-config integration) AND overlay exists
if [ -f "/etc/platform-config/kustomize/kustomize.sh" ] && [ -d "/etc/platform-config/kustomize/stackstorm/overlay" ]; then
    HELM_CMD="$HELM_CMD \
  --post-renderer /etc/platform-config/kustomize/kustomize.sh \
  --post-renderer-args stackstorm/overlay"
fi

# Add values files (these override the external services config if needed)
if [ -f "/etc/platform-config/helm-configs/stackstorm/stackstorm-helm-overrides.yaml" ]; then
    HELM_CMD="$HELM_CMD -f /etc/platform-config/helm-configs/stackstorm/stackstorm-helm-overrides.yaml"
fi

if [ -f "/etc/platform-config/helm-configs/global_overrides/endpoints.yaml" ]; then
    HELM_CMD="$HELM_CMD -f /etc/platform-config/helm-configs/global_overrides/endpoints.yaml"
fi
