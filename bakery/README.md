...
- If `queue` is not provided, Bakery uses `RACKSPACE_CORE_DEFAULT_QUEUE` (default `Support Queue`).
...
| `RACKSPACE_CORE_URL` | Rackspace Core | CTKAPI base URL (default: `https://core.example.com`) |
...
| `RACKSPACE_CORE_DEFAULT_QUEUE` | Rackspace Core | Default queue used when route `provider_config.queue` is not provided (default: `Support Queue`) |
