...
VERSION_FILE_PRIMARY="/etc/platform-config/helm-chart-version.yaml"
VERSION_FILE_FALLBACK="/srv/config/chart-versions.yaml"
VERSION_FILE_MANAGED="/srv/config/chart-versions.yaml"
GLOBAL_OVERRIDES_DIR="/etc/platform-config/helm-configs/global_overrides"
POUNDCAKE_CONFIG_DIR="/srv/config/poundcake"
STACKSTORM_CONFIG_DIR="/etc/platform-config/helm-configs/stackstorm"
POUNDCAKE_BASE_OVERRIDES="/opt/platform-config/base-helm-configs/poundcake/poundcake-helm-overrides.yaml"
STACKSTORM_BASE_OVERRIDES="/opt/platform-config/base-helm-configs/stackstorm/stackstorm-helm-overrides.yaml"
STACKSTORM_DEFAULT_OVERRIDES="${HELM_ROOT}/stackstorm/values-external-services.yaml"
KUSTOMIZE_RENDERER="/etc/platform-config/kustomize/kustomize.sh"
KUSTOMIZE_OVERLAY_DIR="/etc/platform-config/kustomize/poundcake/overlay"
...
if [[ -z "${POUNDCAKE_VERSION}" ]]; then
  echo "Error: could not determine PoundCake chart version. Set /srv/config/chart-versions.yaml key 'poundcake' or env POUNDCAKE_CHART_VERSION." >&2
  exit 1
fi
